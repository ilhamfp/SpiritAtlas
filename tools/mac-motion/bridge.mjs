...
const sessionHours=Number(process.env.BTB_SESSION_HOURS??8);
if(!Number.isInteger(sessionHours)||sessionHours<1||sessionHours>24)throw new Error('BTB_SESSION_HOURS must be a whole number from 1 to 24.');
const token=randomBytes(32).toString('base64url'),session=randomBytes(16).toString('hex'),expires=Date.now()+sessionHours*60*60_000;
...
const reader=spawn(path.join(root,'tools/mac-motion/.build/btb-motion-reader'),['--seconds',String(sessionHours*3600)],{stdio:['ignore','pipe','pipe']});
