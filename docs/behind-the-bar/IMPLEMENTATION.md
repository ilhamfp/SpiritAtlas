...
## Native streaming correction

Fetch chunk boundaries are unrelated to NDJSON record boundaries. The receiver parses complete lines before bounding the remaining unfinished line to 2048 bytes. Large batches of valid complete reports remain connected; per-packet sequence, session, timestamp, unit, finite-value and 250 ms freshness checks remain in force. Old buffered reports cannot rearm or resume pouring. Malformed unfinished data aborts the fetch so the helper releases its single subscriber. Regression fixtures explicitly cover coalesced chunks, split lines, stale buffered input and malformed remainder. The actual-device recovery script separately stalls the normal Chrome page 2.5 seconds and verifies real incoming reports recover disarmed.

Final native/performance verification uses installed Chrome with a private task profile and normal security, sandbox, media and focus defaults. CDP attaches with `noDefaults:true`; no programmatic permission grant or focus emulation. `btb-native-session.mjs` saves incomplete attempts and closes its owned browser on timeout. Its screen recording is useful for visible state and phase alignment but does not itself film physical laptop movement.
