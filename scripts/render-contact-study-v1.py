...
p=argparse.ArgumentParser();p.add_argument('--lighting',choices=['neutral','source'],default='neutral');p.add_argument('--density',type=float,default=.55);p.add_argument('--render',action='store_true');a=p.parse_args(sys.argv[sys.argv.index('--')+1:] if '--' in sys.argv else [])
...
if a.lighting=='source':
    stage_source=P/'assets/blender/experiments/negroni-express-v012-interfaces-source-unified-d35-hero.blend'
...
label='contact-'+a.lighting+'-d'+format(a.density,'g').replace('.','p');out=P/f'qa/evidence/blender/negroni-express-hero-v012-{label}.png';scene.render.filepath=str(out)
