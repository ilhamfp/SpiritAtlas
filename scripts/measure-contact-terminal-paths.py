...
for name, center in regions.items():
    paths = [trace((center[0] + random_source.uniform(-5, 6), center[1] + random_source.uniform(-5, 6))) for _ in range(1024)]
...
report = dict(source=str(source.relative_to(ROOT)), sourceSha256=hashlib.sha256(source.read_bytes()).hexdigest(),
    ...
    view=args.view, stageRepresentation='Actual evaluated stage_counter and stage_square_coaster triangles, including their original world positions. No substitute analytic floor plane. Counts are camera-path terminal objects under geometric-normal specular sampling, not Cycles light/shadow path endpoints or radiance fractions.',
    ...
    regions=summaries)
output = ROOT / (args.output or f'qa/evidence/blender/v012-contact-{args.view}-terminal-paths.json')
