...
p=argparse.ArgumentParser();p.add_argument('--lighting',choices=['neutral','source'],default='neutral');p.add_argument('--view',choices=['hero','photo'],default='hero');p.add_argument('--density',type=float,default=.55);p.add_argument('--render',action='store_true');a=p.parse_args(sys.argv[sys.argv.index('--')+1:] if '--' in sys.argv else [])
...
label='contact-'+a.lighting+'-d'+format(a.density,'g').replace('.','p');out=P/f'qa/evidence/blender/negroni-express-{a.view}-v012-{label}.png';scene.render.filepath=str(out)
