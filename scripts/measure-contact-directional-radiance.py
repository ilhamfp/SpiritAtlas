...
for name, center in regions.items():
    ...
    environment=[path for path in completed if path['terminal']=='environment' and not path['mismatches']]
    ...
    summaries[name] = dict(cropPixelCenter=center, sampleCount=len(paths),
        ...
        invalidPaths=[path for path in paths if path['mismatches']],
        environmentInvalidPathsExcluded=sum(path['terminal']=='environment' and bool(path['mismatches']) for path in paths),
