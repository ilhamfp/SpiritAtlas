...
Run Blender --background --python scripts/build-assets.py -- --drink negroni-express --render
All geometry is independently authored. World: Z up; 1 scene unit ≈ 4 cm (estimated, not measured).
"""
import bpy, math, random, argparse, sys, json, hashlib, struct
...
p.add_argument('--output-tag',default='',help='Optional isolated candidate subdirectory; leaves production scenes/GLBs untouched.')
...
def pepper():
 ...
 skin=mat('Shishito wet olive skin',(.13,.17,.02,1),.36,noise=.007)
 bs=skin.node_tree.nodes.get('Principled BSDF');bs.inputs['Coat Weight'].default_value=.10;bs.inputs['Coat Roughness'].default_value=.30
 ...
 vs=[];fs=[];cs=[];long=240;around=72
 for i in range(long+1):
  t=i/long;x=-.56+1.60*t;y=-.10-.42*math.sin(math.pi*t);z=1.965-.020*math.sin(math.pi*t)+.37*t*t
  # broad shoulder at stem, narrowing toward hooked tip
  r=(.055+.106*math.sin(math.pi*t)**.70)*(1-t**10)+.040
  if t<.05:r*=math.sqrt(max(.001,1-((.05-t)/.05)**2))
  # Hemispherical final cap removes the visibly sliced end without sharpening the tip.
  if t>.94:r*=math.sqrt(max(.001,1-((t-.94)/.06)**2))
  for j in range(around):
   th=j*math.tau/around
   rib=1+.085*math.cos(5*th+.38*math.sin(8*t))+.052*math.cos(3*th-1.2+math.sin(9*t))+.021*math.cos(9*th+2.2*t)
   fold=1+.027*math.sin(43*t+2.3*math.sin(th*3))+.012*math.sin(89*t-5*th)
   # Individual blunt pickling creases interrupt the longitudinal valleys.
   for ct,ph,depth,width in [(.18,1.2,.10,.044),(.31,3.4,.12,.031),(.47,5.1,.085,.037),(.61,2.6,.11,.040),(.78,.1,.09,.028)]:
   ...
   cs.append((.125*shade,.170*shade,.021*shade,1))
 for i in range(long):
  for j in range(around):
 ...
 curve('garnish_stem',[(-.57,-.10,1.98),(-.80,-.06,2.08),(-1.12,-.035,2.27),(-1.42,.0,2.37),(-1.49,.035,2.44)],.021,stem,category='garnish',lift=5.80)
 for k in range(6):
  th=k*math.tau/6
  curve('garnish_calyx_'+str(k),[(-.64,-.115,2.005),(-.56,-.10+.064*math.sin(th),1.975+.05*math.cos(th)),(-.44,-.10+.09*math.sin(th),1.985+.07*math.cos(th))],.009,stem,category='garnish',lift=5.80)

def orange():
 rind=mat('Orange aromatic bowed rind',(.62,.145,.006,1),.38,noise=.009)
 rind.node_tree.nodes.get('Principled BSDF').inputs['Specular IOR Level'].default_value=.20
 ...
 for side in [0,1]:
  for i in range(nr+1):
   ...
   for j in range(nt):
    ...
    z+=(.008*math.cos(3*th+.7)+.006*math.sin(7*th-1.2))*math.sin(math.pi*i/nr)
    rr=r*(1+.014*math.sin(3*th)+.009*math.cos(7*th-.8))
...
def flower():
 ...
 yellow=[]
 for i in range(7):
  m=mat('Yellow petals '+str(i),(.72+i*.020,.39+i*.025,.006+i*.0015,1),.50,noise=.00065)
 ...
 for ring in range(8):
  count=13+ring*3;rad=.020+ring*.032
  for k in range(count):
   theta=k*math.tau/count+ring*.67+random.uniform(-.070,.070)
   length=(.065+.012*ring)*random.uniform(.80,1.16);width=(.014+ring*.003)*random.uniform(.80,1.20)
   tilt=random.uniform(-.010,.042)+.030*(1-ring/8);curl=random.uniform(-.008,.018);twist=random.uniform(-.25,.25)
   basez=2.010+.105*(1-ring/8)+random.uniform(-.012,.012)
   vs=[];fs=[]
   for layer in [0,1]:
    for i in range(15):
     t=i/14
     for j in range(7):
      w=(j/6-.5)*2;rr=rad+length*t-width*.8*(1-math.sqrt(max(0,1-w*w)))*t**9;ang=theta+twist*t*t
      # Broad spoon end with rounded perimeter, not a pointed leaf silhouette.
      shape=.35+.65*math.sin(math.pi*.5*min(t/.65,1))
      cross=width*shape*w
      z=basez+.035*math.sin(t*math.pi)+tilt*t+curl*t**3+.012*w*w*math.sin(math.pi*t)
      z+=.004*math.sin(t*17+theta+4*w)*math.sin(math.pi*t)+.007*w*t*math.sin(t*math.pi)+layer*.0023
      vs.append((.25+rr*math.cos(ang)-cross*math.sin(ang),.06+rr*math.sin(ang)+cross*math.cos(ang),z))
   per=15*7
   for layer in [0,1]:
    for i in range(14):
     for j in range(6):
      n=layer*per+i*7+j;f=(n,n+1,n+8,n+7);fs.append(f if layer==0 else tuple(reversed(f)))
   # Close both long sides and both ends; every petal retains its real back surface.
   for i in range(14):
    n=i*7;fs.append((n,n+7,n+7+per,n+per));n=i*7+6;fs.append((n,n+per,n+7+per,n+7))
   for j in range(6):
    fs.append((j,j+per,j+per+1,j+1));n=14*7+j;fs.append((n,n+1,n+1+per,n+per))
   mesh('garnish_petal_%02d_%02d'%(ring,k),vs,fs,random.choice(yellow),'garnish',lift=5.75)
 for i in range(35):
  theta=i*2.399;r=.059*math.sqrt(i/35)
  sphere('garnish_floret_%03d'%i,(.25+r*math.cos(theta),.06+r*math.sin(theta),2.145+random.uniform(-.011,.011)),(.006,.006,.012),yellow[2],category='garnish',lift=5.75)
...
def build(drink):
 ...
 if drink=='negroni-express':pepper()
