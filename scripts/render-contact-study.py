...
if a.world_strength_scale<=0:p.error('World strength scale must be positive.')
if a.world_strength_scale!=1 and a.lighting!='source':p.error('Broad world illumination treatment requires source lighting.')
if a.directional_world and (a.lighting!='source' or a.world_strength_scale!=1):p.error('Directional environment requires source lighting and the unscaled source world.')
...
world_strength=scene.world.node_tree.nodes.get('Background').inputs['Strength'];previous_world_strength=world_strength.default_value;world_strength.default_value*=a.world_strength_scale
...
directional_treatment=None
if a.directional_world:
    import importlib.util
    config_path=P/'assets/blender/experiments/somma-directional-world-v1.json';config=json.loads(config_path.read_text());spec=importlib.util.spec_from_file_location('directional_world',P/'scripts/directional-world.py');module=importlib.util.module_from_spec(spec);spec.loader.exec_module(module)
    directional_treatment=module.install(scene.world,config);directional_treatment.update(configPath=str(config_path.relative_to(P)),configSha256=sha(config_path))
label='contact-'+a.lighting+'-d'+format(a.density,'g').replace('.','p')+('-white-coaster' if a.white_coaster else '')+('-world'+format(a.world_strength_scale,'g').replace('.','p') if a.world_strength_scale!=1 else '')+('-directional-world-v1' if a.directional_world else '');out=P/f'qa/evidence/blender/negroni-express-{a.view}-v012-{label}.png';scene.render.filepath=str(out)
...
record['treatment']['worldIllumination']=dict(strengthBefore=previous_world_strength,strengthScale=a.world_strength_scale,strengthAfter=world_strength.default_value,colorUnchanged=True,directLightsUnchanged=True,meaning='Single source-world strength multiplier. No exposure, density, direct-light, geometry or surface-material compensation.')
record['treatment']['directionalEnvironment']=directional_treatment
record['lighting']['worldStrengthIsLinked']=world_strength.is_linked
