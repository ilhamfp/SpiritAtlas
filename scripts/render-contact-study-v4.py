...
label='contact-'+a.lighting+'-d'+format(a.density,'g').replace('.','p')+('-white-coaster' if a.white_coaster else '')+('-world'+format(a.world_strength_scale,'g').replace('.','p') if a.world_strength_scale!=1 else '');out=P/f'qa/evidence/blender/negroni-express-{a.view}-v012-{label}.png';scene.render.filepath=str(out)
