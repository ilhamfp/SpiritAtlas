...
import { loadDropletMask } from './core/droplet-mask.js';
...
export async function createNegroniRenderer(container, options = {}) {
  ...
  let controlledOrbit;
  let lastDraw = 0, fpsStart = 0, fpsFrames = 0, measuredFps = null, renderedFrames = 0;
  let currentRig = 0;
  const projected = new THREE.Vector3();
  const anchors = {gin:new THREE.Vector3(-1.75,3.65,0),campari:new THREE.Vector3(.1,4.9,-.1),vermouth:new THREE.Vector3(1.9,3.25,.1)};
  ...
  function cameraPose(reset = false) {
    ...
    if (controlledOrbit) {
      const elevation = Math.atan(.38) + controlledOrbit.elevation - .16;
      const radius = distance / controlledOrbit.zoom;
      const phi = THREE.MathUtils.clamp(Math.PI / 2 - elevation, controls.minPolarAngle, controls.maxPolarAngle);
      const theta = THREE.MathUtils.clamp(controlledOrbit.azimuth, controls.minAzimuthAngle, controls.maxAzimuthAngle);
      camera.position.copy(target).add(new THREE.Vector3().setFromSpherical(new THREE.Spherical(radius,phi,theta)));
      controls.target.copy(target);
    } else if (reset || !hasOrbited) {
  ...
  function projectAnchors(){
    if(!garnish||!camera)return {};
    const points={...anchors,...garnish.anchors},result={};
    for(const [name,point] of Object.entries(points)){
      projected.copy(point);projected.y*=MODEL_HEIGHT;projected.project(camera);
      result[name]={x:(projected.x*.5+.5)*container.clientWidth,y:(-.5*projected.y+.5)*container.clientHeight,visible:projected.z>-1&&projected.z<1&&Math.abs(projected.x)<1.2};
    }
    return result;
  }
  ...
  function drawFrame(now) {
    ...
    try {
      ...
      if (poseDirty) {
        ...
        const rigProgress = currentRig = requestedRig ?? particleSolver.rigProgress;
      ...
      if (renderNeeded || orbitMoving) {
        ...
        renderer.render(scene, camera);
        renderedFrames++;
        if(!lastDraw || now-lastDraw>250){fpsStart=now;fpsFrames=0;measuredFps=null;}
        lastDraw=now;fpsFrames++;
        if(now-fpsStart>=1000){measuredFps=fpsFrames*1000/(now-fpsStart);fpsStart=now;fpsFrames=0;}
        Object.assign(renderer.domElement.dataset,{renderedFrames:String(renderedFrames),renderedAt:String(now),fps:measuredFps===null?'':measuredFps.toFixed(1),fpsMethod:'completed-webgl-frame',drinkId:options.id||'classic-negroni'});
        options.onFrame?.({progress:sampleProgress,rigProgress:currentRig,renderedFrames,renderedAt:now,fps:measuredFps,anchors:projectAnchors()});
      }
  ...
  try {
    ...
    const dropletMask = await loadDropletMask({cache:solver,signal:abortController.signal});
    checkActive();
    ...
    renderer.setPixelRatio(Math.min(window.devicePixelRatio || 1, options.maxPixelRatio ?? 1.7));
    ...
    canvas.setAttribute('aria-label', options.label || 'Interactive classic Negroni');
    ...
    canvas.dataset.dropletMaskSha256 = dropletMask.metadataSha256;
    ...
    particleSurface = createParticleSurface(THREE, particleSolver, {...options.palette,dropletMask});
    scene.add(particleSurface.group);
    garnish = options.garnishFactory ? options.garnishFactory(THREE, createGarnish) : createGarnish(THREE);
    ...
    const readOrbit = () => {
      ...
      return {azimuth:sphere.theta,elevation:Math.PI/2-sphere.phi-Math.atan(.38)+.16,zoom:lastCameraDistance/Math.max(.0001,sphere.radius)};
    };
    const onOrbitStart = () => { hasOrbited = true; controlledOrbit=undefined; options.onOrbit?.(); invalidate(); };
    const onOrbitEnd = () => options.onOrbitChange?.(readOrbit());
    ...
    return {
      get state(){return {progress:sampleProgress,rigProgress:currentRig,renderedFrames,fps:measuredFps,visible,disposed};},
      getAnchors:projectAnchors,
      getOrbit:readOrbit,
      setOrbit(orbit){
        if(disposed)return;
        for(const key of ['azimuth','elevation','zoom'])if(!Number.isFinite(orbit[key]))throw new Error('Invalid classic orbit');
        controlledOrbit={azimuth:orbit.azimuth,elevation:orbit.elevation,zoom:THREE.MathUtils.clamp(orbit.zoom,.65,1.8)};
        hasOrbited=true;cameraPose();controls.update();invalidate();
      },
      ...
      resetCamera() {
        if (disposed) return;
        hasOrbited = false;controlledOrbit=undefined;
