...
import {MAX_MOTION_SESSION_MS} from './pairing';
...
export class NativeMotion {
 ...
 validate(p:unknown):p is NativePacket{const v=p as NativePacket;return !!v&&v.v===1&&v.kind==='native-spu'&&v.unit==='g-deg/s'&&typeof v.session==='string'&&/^[a-f0-9]{32}$/.test(v.session)&&Number.isSafeInteger(v.seq)&&v.seq>this.lastSeq&&Number.isFinite(v.t)&&v.t>this.lastT&&Number.isFinite(v.expires)&&v.expires>Date.now()&&v.expires-Date.now()<=MAX_MOTION_SESSION_MS&&(v.sampleEpoch===undefined||(Number.isFinite(v.sampleEpoch)&&Math.abs(Date.now()-v.sampleEpoch)<1000))&&Array.isArray(v.accel)&&Array.isArray(v.gyro)&&v.accel.length===3&&v.gyro.length===3&&v.accel.every(n=>Number.isFinite(n)&&Math.abs(n)<8)&&v.gyro.every(n=>Number.isFinite(n)&&Math.abs(n)<2000)&&(!this.session||v.session===this.session);}
