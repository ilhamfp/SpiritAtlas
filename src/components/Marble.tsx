 ...
 useEffect(()=>{
  ...
  function draw(){
   ...
   for(let y=0,row=0;y<h;y+=pitch,row++)for(let x=0,col=0;x<w;x+=pitch,col++){
    const u=x/w,v=y/h;
    // Keep the entire flowing image, dissolving only its lower contour.
    // The former right-only mask hid the left ribbon and left fragments
    // behind the player. Coverage now follows the full-width composition.
    const dissolveStart=mobile ? .83+.035*Math.sin(u*8) : .76+.08*Math.sin(u*5);
    const coverage=Math.max(0,Math.min(1,(1-v)/(1-dissolveStart)));
    if(coverage>=.985)ctx.rect(x,y,pitch,pitch);
    else if(coverage>(BAYER[(row%4)*4+col%4]+.5)/16){const size=pitch*Math.sqrt(coverage);ctx.rect(x+(pitch-size)/2,y+(pitch-size)/2,size,size)}
   }
   ctx.fill();ctx.globalCompositeOperation='source-over';
  }
  const observer=new ResizeObserver(()=>{
   if (!canvas.clientWidth || !canvas.clientHeight) return;
   const src=canvas.clientWidth<=PORTRAIT_WIDTH?'/textures/spiritatlas-marble-base-mobile-v1.webp':'/textures/spiritatlas-marble-base-desktop-v1.webp';
   if(src!==loadedSrc){loadedSrc=src;const img=new Image();img.onload=()=>{if(!disposed&&src===loadedSrc){texture=img;draw()}};img.src=src}else draw();
