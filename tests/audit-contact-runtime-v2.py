...
manifest=json.loads((D/'manifest.json').read_text());source=json.loads((D/'independent-source.json').read_text());closure_source=json.loads((D/'independent-source-closure.json').read_text());assert sha(P/closure_source['binary'])==closure_source['binarySha256'];closure=np.fromfile(P/closure_source['binary'],dtype='<f4').reshape(-1,18).astype(float);closure_used=set();closure_report=[]
...
for mi,mat in enumerate(j['materials']):
 ...
 assert boundary['schemaVersion']==1;omit=boundary['omitFromOpticalTrace'];assert (boundary['insideMedium']==boundary['outsideMedium'])==omit;inside=media[str(boundary['insideMedium'])]['ior'];outside=media[str(boundary['outsideMedium'])]['ior'];assert abs(boundary['relativeIOR']-inside/outside)<1e-14;standard=mat.get('extensions',{}).get('KHR_materials_ior',{}).get('ior',1.5);assert standard>=1 and abs(standard-(1 if omit else inside))<1e-7
 all_materials.append({'index':mi,'name':mat['name'],'inside':boundary['insideMedium'],'outside':boundary['outsideMedium'],'relativeIOR':boundary['relativeIOR'],'standardIOR':standard,'omitFromOpticalTrace':omit})
def walk(ni,parent):
 ...
 if 'mesh' in node and name in names:
  ...
  used=set();num=0;maxpos=0.;maxnorm=0.;maxgeonorm=0.;nonexactnorm=0;unmatched=[];regioncounts=collections.Counter();localmisses=0;zeroSourceNormals=0;zeroExportNormals=0;localNormalDifferences=[];degenerateSourceWorld=0;degenerateActualWorld=0;nonzeroNormalMax=0.;nonzeroNormalCornerFailures=0;zeroNormalFallbacks=[];nm=np.linalg.inv(world[:3,:3]).T
  for primitive in j['meshes'][node['mesh']]['primitives']:
   assert primitive.get('mode',4)==4;attributes=primitive['attributes'];vp=accessor(attributes['POSITION']);vn=accessor(attributes['NORMAL']);localvp=vp.copy();localvn=vn.copy();vp=vp@world[:3,:3].T+world[:3,3];vn=vn@nm.T;vn/=np.linalg.norm(vn,axis=1)[:,None];ids=accessor(primitive['indices']).astype(int).reshape(-1,3);mat=j['materials'][primitive['material']];boundary=mat['extras']['atlasOpticalBoundary'];label=(boundary['insideMedium'],boundary['outsideMedium'])
   if boundary['omitFromOpticalTrace']:
    assert name=='ice' and label==(3,3) and boundary['relativeIOR']==1
    mapping=collections.defaultdict(list)
    for ci,t in enumerate(closure[:,:9].reshape(-1,3,3)):
     for shift in range(3):mapping[tuple(np.roll(t,-shift,axis=0).reshape(-1))].append((ci,shift))
    for indices in ids:
     matches=[v for v in mapping.get(tuple(localvp[indices].reshape(-1)),[]) if v[0] not in closure_used];assert matches,'Closure position mismatch';ci,shift=matches[0];closure_used.add(ci)
    closure_report.append({'object':name,'material':mat['name'],'triangles':len(ids),'labels':list(label),'relativeIOR':boundary['relativeIOR']});continue
   regioncounts[str(label)]+=len(ids)
   for indices in ids:
    ...
    ln=np.roll(localnormals[ci],-shift,axis=0);lnlen=np.linalg.norm(ln,axis=1);valid=lnlen>0;normalLocal=localvn[indices];normalizedSource=np.divide(ln,lnlen[:,None],out=np.zeros_like(ln),where=lnlen[:,None]!=0);normalizedActual=normalLocal/np.linalg.norm(normalLocal,axis=1)[:,None];componentErrors=np.max(np.abs(normalizedSource-normalizedActual),axis=1)
    if np.any(valid):nonzeroNormalMax=max(nonzeroNormalMax,float(np.max(componentErrors[valid])));nonzeroNormalCornerFailures+=int(np.sum(componentErrors[valid]>1e-3))
    if not np.all(valid):zeroNormalFallbacks.append({'sourceTriangle':int(ci),'corners':np.where(~valid)[0].tolist(),'actualNormals':normalLocal[~valid].tolist()})
    ld=float(np.max(np.abs(ln-localvn[indices])))
